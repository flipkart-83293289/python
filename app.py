...
# NOTE: resets whenever the Render service restarts/spins down (free tier).
user_state = {}
...
def set_webhook():
    """
    Visit this once in your browser, after the app is deployed:
    https://<your-render-url>/set_webhook
    It builds the correct webhook URL itself, so there's no chance of typos.
    """
...
def webhook():
    ...
    if text == "/start" or state["stage"] == "idle":
        ...
        if not filtered:
            send_message(chat_id, f"No results for '{product}' between ₹{low} and ₹{high}.")
        else:
            for item in filtered[:10]:
                caption = (
                    f"{item['name']}\n"
                    f"₹{item['current_price']} (was ₹{item['original_price']})\n"
                    f"{item['link']}"
                )
